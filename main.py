...
from guidance.sd import StableDiffusion, StableDiffusionLora
from utils import *
...
def init_model(args):
    model = StableDiffusionLora(args, use_lora=True, t_range=[0.02, 0.98])
...
def run(args):
    ...
    if args.loss_type in ["pds"]:
        ...
        # prev_latents = deepcopy(latents).requires_grad_(False)

    if args.loss_type == 'vds':
        lora_layers = model.use_lora()
        # _ = model.use_lora()
        # lora = torch.tensor(list(lora_layers))
    ...
    for step in tqdm(range(steps)):
        ...
        elif args.loss_type == "vds":

            loss, eps, t = model.get_vds_loss(
                latents=latents,
                text_embeddings=text_embeddings, 
                guidance_scale=guidance_scale,
            )
    ...
    img = model.decode_latents(latents)
    if args.loss_type == "sds" or args.loss_type == 'vds':
...
def main():
    args = parse_args()
    assert args.loss_type in ["sds", "pds", "vds"], "Invalid loss type"
    if args.loss_type in ["pds"]:
